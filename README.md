...
| `enrich_details.py` | Eksik puan/indirme/boyut/Android surumunu Play Store, Aptoide, Steam ve GitHub'dan doldurur (gunluk). `--refresh` ile hepsini yeniler ve Play'den kalkan oyunlari isaretler (haftalik). Veri yoksa uydurmaz. |
...
## Otomatik guncelleme (GitHub Actions)

| Workflow | Ne zaman | Ne yapar |
|---|---|---|
| `update-apks.yml` | Her gun 04:00 UTC | APK linklerini kontrol eder, eksik bilgileri doldurur, siralamayi korur |
| `weekly-refresh.yml` | Pazartesi 03:00 UTC | Puan, indirme, surum, yas siniri ve "Yenilikler"i yeniler; Play'den kalkan oyunlari isaretler |
| `sync-main.yml` | master'a her push | `main`'i esitler (uygulama `@main` okur) |

Hepsi Actions sekmesinden elle de calistirilabilir.
